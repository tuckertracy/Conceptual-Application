...
public class HuffmanCode {
    ...
    public HuffmanCode(InputStreamReader isr) throws IOException {
        tree = readTree(isr);
        code = new String[256]; // may need to change
        buildCode(tree, "");
    }
    ...
    public char decodeNext(InputStreamReader in) throws IOException {
        return decodeNode(in,this.tree);
    }
    ...
    private char decodeNode(InputStreamReader in, Node cur) throws IOException {
        ...
        if ( ch == '0' ) {
            return decodeNode(in, cur.left);
        }

        return decodeNode(in, cur.right);
    }
}
