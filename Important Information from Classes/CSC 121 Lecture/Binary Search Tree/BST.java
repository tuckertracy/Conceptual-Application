...
public class BST<Key extends Comparable<Key>, Value> implements Iterable<Key> {
    ...
    private class Node {
        ...
        private int size;          // number of nodes in subtree (cached)
    ...
    public Iterable<Key> levelOrder() {
        Queue<Key> keys = new ArrayDeque<Key>();
    ...
    public Key lowestCommonAncestor(Key k1, Key k2) {
        return lowestCommonAncestor(root, k1, k2);
    }

    private Key lowestCommonAncestor(Node cur, Key k1, Key k2) {
        if ( (k1 == null || k2 == null) || (cur == null) || 
        ...
        if ( k1.equals(cur.key) || k2.equals(cur.key) ) {
            return cur.key;
        }

        if ( (k1.compareTo(cur.key) > 0 && k2.compareTo(cur.key) < 0)
        || (k2.compareTo(cur.key) > 0 && k1.compareTo(cur.key) < 0) ) {
            return cur.key;
        }

        if ( (k1.compareTo(cur.key) < 0 && k2.compareTo(cur.key) < 0) ) {
            return lowestCommonAncestor(cur.left, k1, k2);
        }

        if ( (k1.compareTo(cur.key) > 0 && k2.compareTo(cur.key) > 0) ) {
            return lowestCommonAncestor(cur.right, k1, k2);
        }
